...
export default function App() {
  ...
  React.useEffect(() => { setChapterIndex(0); setSubIndex(0); }, [activeBlockId]);
  React.useEffect(() => { setSubIndex(0); }, [chapterIndex]);

  const pct = getBlockPct(activeBlock);
  const fillStyle = { width: `${pct}%`, background: colorFromPct(pct, activeBlock.color) };

  const chapter = activeBlock.chapters[chapterIndex];
  const subtopic = chapter.subtopics[subIndex];

  const readWholeChapter = () => {
    const items = chapter.subtopics.map(st => `${st.title}. ${st.summary}`);
    tts.speakMany(items);
  };
            ...
            <div className="border rounded-xl p-3 bg-gradient-to-tr from-blue-100 to-white">
              <div className="flex items-center gap-3">
                ...
                <div className="min-w-0">
                  ...
                  <p className="text-slate-700">Subtemas navegables con audio y quiz.</p>
                    ...
                    <div className="flex items-center gap-1 text-xs">
                      ...
                      {activeBlock.chapters.map((ch, i) => (
                        <button
                          ...
                          className={`ml-1 rounded-full px-3 py-1 border ${i === chapterIndex ? "bg-blue-600 text-white border-blue-600" : "bg-white"}`}
                    ...
                    <div className="flex items-center gap-1 text-xs">
                      ...
                      {chapter.subtopics.map((st, i) => (
                        <button
                          ...
                          className={`ml-1 rounded-full px-2 py-1 border ${i === subIndex ? "bg-slate-800 text-white border-slate-800" : "bg-white"}`}
                          aria-label={`Ir al subtema ${i + 1}: ${st.title}`}
                        >
                          {i + 1}
                        </button>
                      ))}
